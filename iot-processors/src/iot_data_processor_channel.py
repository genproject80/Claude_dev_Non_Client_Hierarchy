...
class IoTDataProcessor:
    ...
    def process_channel(self, channel_devices: pd.DataFrame, client_id: int) -> int:
        """Process a channel once and map data to correct devices based on field IDs."""
        if channel_devices.empty:
            return 0
            
        channel_id = channel_devices['Channel_ID'].iloc[0]
        api_key = channel_devices['APIKey'].iloc[0]
        
        self.logger.info(f"Processing Channel {channel_id} with {len(channel_devices)} devices")
        
        # Create device mapping: field_id -> device_info
        device_mapping = {}
        for _, device_row in channel_devices.iterrows():
            field_id = device_row['Field_ID']
            device_mapping[field_id] = {
                'device_id': device_row['Device_ID'],
                'logic_id': device_row['ConversionLogicID'],
                'table_name': device_row['TransactionTableName']
            }
        
        # Get the earliest last_created_at across all devices in this channel
        earliest_timestamp = None
        for _, device_row in channel_devices.iterrows():
            device_id = device_row['Device_ID']
            table_name = device_row['TransactionTableName']
            last_created = self.get_last_created_at(device_id, table_name)
            
            if last_created and (not earliest_timestamp or last_created < earliest_timestamp):
                earliest_timestamp = last_created
        
        start_time = earliest_timestamp.isoformat() if earliest_timestamp else "2025-01-01T00:00:00Z"
        
        # Fetch channel data once
        url = f"{self.config['thingspeak']['base_url']}/channels/{channel_id}/feeds.json"
        params = {
            'api_key': api_key,
            'results': 8000
        }
        ...
        try:
            self.session_stats['api_calls_made'] += 1
            response = requests.get(url, params=params, timeout=self.config['thingspeak'].get('timeout', 30))
            response.raise_for_status()
            
            data = response.json()
            feeds = data.get('feeds', [])
            
            self.logger.info(f"Fetched {len(feeds)} channel records from ThingSpeak for channel {channel_id}")
            
        except requests.exceptions.RequestException as e:
            self.logger.error(f"Failed to fetch channel data for channel {channel_id}: {str(e)}")
            self.session_stats['errors_encountered'] += 1
            return 0
        
        if not feeds:
            self.logger.info(f"No new data for channel {channel_id}")
            return 0
        
        # Group decoded records by device and table
        device_records = {}
        
        for feed in feeds:
            ts = pd.to_datetime(feed["created_at"])
            
            # Scan fields 1-8 to find data and match to devices
            for i in range(1, 9):
                field_key = f'field{i}'
                hex_val = feed.get(field_key)
                
                if hex_val and i in device_mapping:
                    device_info = device_mapping[i]
                    device_id = device_info['device_id']
                    logic_id = device_info['logic_id']
                    table_name = device_info['table_name']
                    
                    # Check if this timestamp is newer than last processed for this device
                    ...
                    if decoded_data:
                        ...
                        # Group by device and table
                        key = (device_id, table_name)
                        if key not in device_records:
                            device_records[key] = []
                        device_records[key].append(decoded_data)
        
        # Insert data for each device
        total_rows_inserted = 0
        for (device_id, table_name), records in device_records.items():
            rows_inserted = self.insert_data_to_table(records, table_name, device_id)
            total_rows_inserted += rows_inserted
        
        return total_rows_inserted

    def process_device(self, device_row: pd.Series) -> int:
        """Process a single device and return number of rows inserted."""
        device_id = device_row['Device_ID']
        channel_id = device_row['Channel_ID']
        field_id = device_row['Field_ID']  # This may be used for specific field targeting
        api_key = device_row['APIKey']
        logic_id = device_row['ConversionLogicID']
        table_name = device_row['TransactionTableName']
        
        self.logger.info(f"Processing device {device_id} (Channel: {channel_id}, Logic: {logic_id})")
        
        # Get last timestamp
        last_created_at = self.get_last_created_at(device_id, table_name)
        start_time = last_created_at.isoformat() if last_created_at else "2025-01-01T00:00:00Z"
        
        # Fetch channel data (all fields) from ThingSpeak
        url = f"{self.config['thingspeak']['base_url']}/channels/{channel_id}/feeds.json"
        params = {
            'api_key': api_key,
            'results': 8000
        ...
        try:
            ...
            self.logger.info(f"Fetched {len(feeds)} channel records from ThingSpeak for device {device_id}")
            
        except requests.exceptions.RequestException as e:
            self.logger.error(f"Failed to fetch channel data for device {device_id}: {str(e)}")
            self.session_stats['errors_encountered'] += 1
            return 0
        ...
        # Process and decode each feed - scan fields 1-8
        decoded_records = []
            ...
            for i in range(1, 9):
                ...
                if hex_val:
                    ...
                    if decoded_data:
                        ...
                        # For this device, we found data in this field, 
                        # we may want to process only the first matching field per feed
                        # or continue scanning all fields - based on requirements
                        break  # Process only first field with data per feed
            
            if not found_hex:
                self.logger.debug(f"No hex value found in fields 1-8 for entry {feed.get('entry_id')}")
    ...
    def process_client(self, client_id: int) -> Dict[str, int]:
        """Process all devices for a single client by processing each channel once."""
        client_stats = {
            'devices_processed': 0,
            'rows_inserted': 0,
            'api_calls': 0,
        ...
        try:
            ...
            # Group devices by channel to avoid duplicate processing
            for channel_id in devices_df['Channel_ID'].unique():
                channel_devices = devices_df[devices_df['Channel_ID'] == channel_id]
                
                try:
                    rows_inserted = self.process_channel(channel_devices, client_id)
                    client_stats['rows_inserted'] += rows_inserted
                    client_stats['devices_processed'] += len(channel_devices)
                    
                except Exception as e:
                    self.logger.error(f"Error processing channel {channel_id} for client {client_id}: {str(e)}")
